...
session_start();
	...
	if($_SERVER['REQUEST_METHOD'] == "POST")
	{
		...
		if(!empty($Email) && !empty($password) && !is_numeric($user_name))
		{
			...
			//read from database
			$query = "select * from user where user_name = '$Email' limit 1";
...
<body  id="body" class="">
        ...
        <div class="flex justify-center border-rose-500">
            <div class="h-[90%] w-full md:w-3/4 m-4">
                ...
                <button id="c_n_a" class="uppercase px-5 py-2 rounded-md text-white bg-blue-500 hover:bg-violet-600  font-medium"><a href="home.html">login</a></button>
                </div>
            </form>
                ...
                <div class="text-center my-6 flex flex-col " style="margin-left: 20px;">
                    <a href="signup.html" id="c_n_a" class="text-l font-bold text-white hover:ring-sky-500 m-1">Create New Account</a>
                </div>
            </div>
        </div>
    </div>
